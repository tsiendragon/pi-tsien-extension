...
| 6 | 已完成 | `pi-tsien.memory.retrieval.discover.v1`、`search.v1`、`context.coordinator.v1`；Memory coordinator、pi-knowledge provider、无 bridge fallback、deadline；两端 build/typecheck 与 bridge e2e |
...
- promotion 只生成本地 evidence bundle；未获得用户确认不会修改 `marketplace-knowledge`、Marketplace 包或外部仓库。
