...
A goal owns work items and structured blockers. A hard blocker affects only its related items, so independent ready work can continue. The scheduler defaults to a 48-hour wall-clock budget with no turn cap, while retaining consecutive-no-progress safety limits; it reconstructs its counters from branch custom entries after reload and stops with a persisted Markdown report when all paths are blocked. Set `--goal-continuation-max-turns <n>` to add a positive turn cap. The session entries remain canonical; a user-private SQLite materialized ledger at `~/.pi/agent/pi-goal.sqlite` supports restart audit and report storage without deciding branch state.
