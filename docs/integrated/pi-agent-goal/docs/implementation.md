...
It records `goal-continuation` custom entries with source, turn, and no-progress counters, reconstructs unconsumed queues and counters after reload, and defaults to a 48-hour wall-clock budget with no turn cap (`--goal-continuation-max-turns 0`). It stops on stale goal, pause, clear, complete, replacement, user interrupt, duplicate queue, pending messages, busy state, explicit disable, duration budget, no-progress budget, or a positive max-turn cap. When every unfinished item is covered by hard blockers or dependency closure, it persists an `all_paths_blocked` Markdown report in the canonical state and SQLite ledger.
