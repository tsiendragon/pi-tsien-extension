...
- 版本：v0.4
- 日期：2026-08-14
- 范围：Pi TUI、subagent 会话导航、workflow 独立工作台、通用 Sidebar 职责拆分
...
采用两个独立 Extension，禁止重复拥有 subagent/workflow 可视化：

```text
extensions/sidebar.ts
    └─ Main/Pi 会话概览

extensions/subagent-workbench.ts
    └─ 全部 subagent/workflow 可视化和交互
```
...
`subagent-workbench.ts` 是以下信息的唯一可视化入口：
...
当前 `extensions/subagent-sidebar.ts` 同时混合 Main metrics 与 subagent/workflow 状态，不能只改文件名。迁移必须按以下顺序进行：

1. 新建 `subagent-workbench`，先实现现有 agent/workflow 可视化能力。
...
4. 用剩余 Main/Pi 会话能力重新实现 `extensions/sidebar.ts`。
...
- 向 Workbench 暴露版本化、只读 snapshot 和受控 command API；禁止 UI 直接读取 runtime 可变集合或 `__piSubagentura*` 私有全局变量。
...
40. `sidebar.ts` 不读取 `__piSubagentura*`、child session、workflow 或 artifact，未安装 `pi-subagentura` 时仍可独立工作。
41. Agent/Workflow 列表、transcript、usage、activity、Stage 和控制操作只在 `subagent-workbench` 出现。
42. 同一个 agent/workflow 不会被 Sidebar 和 Workbench 同时渲染或重复通知；`sidebar.ts` 的 import graph 不包含 `pi-subagentura` 模块。
...
46. Workbench 只通过版本化 `SubagentWorkbenchRuntime` 获取不可变 snapshot 和发送受控 command，不读取 `__piSubagentura*` 或 runtime 可变对象。
...
- 从旧 Sidebar 删除所有 `pi-subagentura` 状态依赖。
- 重新实现通用 `extensions/sidebar.ts`。
- 注册 `/sidebar` 和 `Ctrl+Alt+S`。
- 移除旧 `subagent-sidebar.ts` 及重复安装说明。
- 验证 Sidebar 未安装 `pi-subagentura` 时仍可独立工作。
...
2. **Sidebar 定位**：保留并重新实现通用 `sidebar`，只展示 Main/Pi 会话概览，不依赖 `pi-subagentura`。
...
8. **Runtime 接口**：Workbench 只通过版本化 snapshot/command API 访问 `pi-subagentura`，不读取私有全局变量。
