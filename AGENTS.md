...
```text
/home/tsien/.pi/agent/extensions.config.json
```

不要在仓库中重新创建 `config/pi-extensions.json`，也不要直接修改
`/home/tsien/.pi/agent/settings.json` 的 `packages` 或 `extensions` 字段，
更不要把托管扩展复制到 `/home/tsien/.pi/agent/extensions/`。这些做法会绕过
严格管理，或导致同一扩展重复加载。
...
- 删除配置项即可停用；同步器会从 Pi 设置移除未列出的 package/extension，
  并隔离未托管的 `/home/tsien/.pi/agent/extensions/` 文件。
