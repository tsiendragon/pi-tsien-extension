...
// (contextWindow - reserveTokens). The two gpt-5.6-luna deployments below
// are explicit exceptions because Codex Luna has a smaller context window.
const ONE_MILLION_TOKENS = 1_000_000;
const DEFAULT_TARGET_PERCENT = 0.4;
const LUNA_TARGET_PERCENT = 0.5;
const LUNA_MODEL_KEYS = new Set([
  "openai-codex/gpt-5.6-luna",
  "azure-okx/gpt-5.6-luna",
]);
...
export default function autoCompactOneMillion(pi: ExtensionAPI): void {
  ...
  const maybeCompact = (_event: unknown, ctx: ExtensionContext): void => {
    ...
    const isLunaModel = modelKey !== undefined && LUNA_MODEL_KEYS.has(modelKey);
    if (!isLunaModel && (!contextWindow || contextWindow < ONE_MILLION_TOKENS)) return;

    if (!usage || usage.percent === null || usage.percent === undefined) return;

    const targetPercent = isLunaModel ? LUNA_TARGET_PERCENT : DEFAULT_TARGET_PERCENT;
    ...
    armed = false;
    ctx.ui.notify(
      `上下文已达 ${Math.round(usage.percent)}%，${isLunaModel ? "gpt-5.6-luna" : "1M 窗口模型"}提前压缩`,
      "info",
    );
    ctx.compact();
  };
