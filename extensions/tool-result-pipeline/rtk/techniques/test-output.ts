...
/**
 * Vendored patch. Upstream matched a bare `test` token, so `ls test`, `cat test`,
 * or `rm -rf test` were treated as test runs and their output was replaced with a
 * synthetic summary. Require an actual test-runner invocation instead, checked at
 * the start of each shell segment so `cd x && npm test` still matches.
 */
const TEST_RUNNERS = [
	// package-manager test scripts, including scoped names like `test:node`
	/^(?:npm|yarn|pnpm|bun)\s+(?:run\s+)?test\b/,
	// node's built-in runner: `node --test`, `node --import tsx --test ...`
	/^node\b[^\n]*\s--test(?:\s|$)/,
	// direct runners, optionally via npx/bunx
	/^(?:npx|bunx)\s+(?:vitest|jest|pytest|mocha|ava|tap)\b/,
	/^(?:vitest|jest|pytest|mocha|ava|tap)\b/,
	/^go\s+test\b/,
	/^cargo\s+test\b/,
];
...
export function isTestCommand(command: string | undefined | null): boolean {
	...
	return command
		...
		.some((segment) => {
			...
			return trimmed.length > 0 && TEST_RUNNERS.some((pattern) => pattern.test(trimmed));
		});
}
