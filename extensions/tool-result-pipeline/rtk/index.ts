...
import { loadConfig, DEFAULT_CONFIG, type RtkConfig } from "./config.ts";
import { trackSavings, getMetricsSummary, clearMetrics } from "./metrics.ts";
import {
	stripAnsiFast,
	truncate,
	filterSourceCode,
...
/**
 * Registers RTK's surface: config loading, the edit-failure system prompt note,
 * the `rtk-*` commands and the `rtk_configure` tool.
 *
 * The `tool_result` transform itself lives in {@link applyRtkFilters} and is
 * driven by `extensions/tool-result-pipeline.ts`, so this layer has exactly one
 * entry point and the stage order is declared in code instead of load order.
 */
export function registerRtkSurface(pi: ExtensionAPI): void {
	let loaded = false;
...
/**
 * The RTK output-filter stage: ANSI stripping, build/test/git/linter aggregation,
 * truncation (bash), source filtering + smart truncation (`read`), and search
 * result grouping (`grep`). Returns `undefined` when nothing changed.
 *
 * Behaviour is unchanged from upstream `pi-rtk` 0.1.4 plus the two local command
 * matcher patches; see `PROVENANCE.md`.
 */
export async function applyRtkFilters(
